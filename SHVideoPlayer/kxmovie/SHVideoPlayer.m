...
@property (nonatomic,strong) KxMovieDecoder * myDecoder;
@property (nonatomic,copy) NSString * myVideoPath;
                               ...
                               parameters: (NSDictionary *) parameters
{
    ...
    return [[SHVideoPlayer alloc] initWithContentPath: path parameters: parameters];
}
...
-(void)commonInit {
    ...
    __weak SHVideoPlayer *weakSelf = self;
    KxMovieDecoder *decoder = [[KxMovieDecoder alloc] init];
    decoder.interruptCallback = ^BOOL(){
        
        __strong SHVideoPlayer *strongSelf = weakSelf;
...
-(void)playWithPath: (NSString *) path {
    if ([self.myVideoPath isEqual:path]) {
        [self setMoviePosition:0];
        [self restorePlay];
        return;
    }
    self.myVideoPath = path;
    if (self.myDecoder) {
        __weak SHVideoPlayer *weakSelf = self;
        dispatch_async(dispatch_get_global_queue(0, 0), ^{

            NSError *error = nil;
            [weakSelf.myDecoder openFile:path error:&error];
                        
            __strong SHVideoPlayer *strongSelf = weakSelf;
            if (strongSelf) {
                dispatch_sync(dispatch_get_main_queue(), ^{
                    [strongSelf setMovieDecoder:strongSelf.myDecoder withError:error];
                });
            }
        });
    }
